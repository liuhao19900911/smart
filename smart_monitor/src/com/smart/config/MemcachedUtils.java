...
public class MemcachedUtils {
	...
	private static final String prdfixKey="";
	...
	private static boolean setExp(String key, Object value, Date expire) {
		boolean flag = false;
		try {
			flag = cachedClient.set(prdfixKey+key, value, expire);
		} catch (Exception e) {
			// 记录Memcached日志
	...
	private static boolean addExp(String key, Object value, Date expire) {
		boolean flag = false;
		try {
			flag = cachedClient.add(prdfixKey+key, value, expire);
		} catch (Exception e) {
			// 记录Memcached日志
	...
	private static boolean replaceExp(String key, Object value, Date expire) {
		boolean flag = false;
		try {
			flag = cachedClient.replace(prdfixKey+key, value, expire);
		} catch (Exception e) {
	...
	public static Object get(String key) {
		Object obj = null;
		try {
			obj = cachedClient.get(prdfixKey+key);
		} catch (Exception e) {
	...
	private static boolean deleteExp(String key, Date expire) {
		boolean flag = false;
		try {
			flag = cachedClient.delete(prdfixKey+key, expire);
		} catch (Exception e) {
	...
	private static class MemcachedLog {
		...
		public static void writeLog(String content) {
			...
			try {
				logWrite.write("[" + PID + "] " + "- [" + DateUtil.format(new Date(), DateUtil.pattern_ymd_hms) + "]\r\n" + content);
				logWrite.newLine();
				logWrite.flush();
			} catch (IOException e) {
